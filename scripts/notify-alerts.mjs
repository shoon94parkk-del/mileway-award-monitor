...
function buildMessages(opened,sourceUpdatedAt){return opened.map(({rule,rows})=>{const visible=rows.slice(0,12),more=rows.length-visible.length;const lines=visible.map(row=>`• ${rowText(row)}`);if(more)lines.push(`• 외 ${more}건`);const ids=rows.map(alertSeatKey).sort().join(',');return {subject:`[Mileway] ${rule.name} 좌석 ${rows.length}건`,text:`🔔 ${rule.name}\n${lines.join('\n')}\n\n대한항공 공개 일일 자료 기준: ${sourceUpdatedAt}\n예약 전 대한항공에서 최종 확인해 주세요.`,idempotency:`${sourceUpdatedAt}|${rule.id}|${ids}`};});}
...
async function readApiRules(){try{const r=await fetch(ALERT_API_URL+'/rules',{headers:{Accept:'application/json'}});if(!r.ok)throw new Error(String(r.status));const data=await r.json();return normalizeAlertRules(data.rules||[]);}catch(error){console.error(`Alert API 규칙 확인 실패: ${error.message}`);return [];}}
...
async function main(){
 ...
 let rules=[];try{const fromSecret=process.env.ALERT_RULES_JSON?normalizeAlertRules(process.env.ALERT_RULES_JSON):[];const fromTelegram=readTelegramRules();const fromApi=await readApiRules();const map=new Map([...fromSecret,...fromTelegram,...fromApi].map(r=>[r.id,r]));rules=[...map.values()];}catch(error){throw new Error(`알림 규칙 읽기 실패: ${error.message}`);}
