...
function buildMessages(opened,sourceUpdatedAt){
 return opened.map(({rule,rows})=>{
  ...
  const lines=visible.map(row=>`• ${rowText(row)}`);if(more)lines.push(`• 외 ${more}건`);
  const ids=rows.map(alertSeatKey).sort().join(',');
...
async function main(){
 ...
 fs.writeFileSync(statePath,JSON.stringify({version:2,source_updated_at:sourceUpdatedAt,channels,updated_at:new Date().toISOString()},null,2)+'\n');
 console.log(sentSummary.length?`알림 처리: ${sentSummary.join(', ')}`:'새롭게 조건에 맞는 좌석 없음');
