...
async function sendEvaluatedRules(rows,rules,prior,sourceUpdatedAt,target){const result=evaluateAlerts(rows,rules,prior),messages=buildMessages(result.opened,sourceUpdatedAt);for(const message of messages)await telegramApi(target.token,'sendMessage',{chat_id:target.chatId,text:message.text,disable_web_page_preview:true});return {result,sentSeats:result.opened.reduce((n,x)=>n+x.rows.length,0)};}
async function notifyAll(input){
 ...
 // Keep the currently configured owner's original alerts alive until their browser opens once and migrates
 // the legacy global rules into its private device namespace. This prevents a migration window with missed alerts.
 const legacyRules=normalizeAlertRules(parseJson(await redis(['GET',LEGACY_STORE_KEY]),[]));
 if(legacyRules.length){const config=await readTelegramConfig();if(config?.token&&config?.default_chat_id){try{const prior=parseJson(await redis(['GET',LEGACY_STATE_KEY]),{}),sent=await sendEvaluatedRules(rows,legacyRules,prior,sourceUpdatedAt,{token:config.token,chatId:String(config.default_chat_id)});await redis(['SET',LEGACY_STATE_KEY,JSON.stringify({version:2,rule_hash:sent.result.rule_hash,rule_meta:sent.result.rule_meta,active:sent.result.active,source_updated_at:sourceUpdatedAt,updated_at:new Date().toISOString()})]);users++;sentSeats+=sent.sentSeats;}catch(error){failures.push(`legacy: ${error.message}`);}}else skippedUnlinked++;}
 for(const owner of owners){const rules=await readRules(owner);if(!rules.length)continue;const target=await chatForOwner(owner);if(!target.chatId||!target.token){skippedUnlinked++;continue;}const prior=await readState(owner);try{const sent=await sendEvaluatedRules(rows,rules,prior,sourceUpdatedAt,target);await writeState(owner,{version:2,rule_hash:sent.result.rule_hash,rule_meta:sent.result.rule_meta,active:sent.result.active,source_updated_at:sourceUpdatedAt,updated_at:new Date().toISOString()});users++;sentSeats+=sent.sentSeats;}catch(error){failures.push(error.message);}}
...
async function restoreBackupIfEmpty(){
 try{await redis(['PING']);const owners=await redis(['GET',OWNERS_KEY]);if(owners)return false;const res=await fetch(BACKUP_URL+'?t='+Date.now(),{cache:'no-store'});if(!res.ok)return false;const record=await res.json(),payload=openConfig(record);if(!payload||payload.version!==2||!Array.isArray(payload.owners))return false;await redis(['SET',OWNERS_KEY,JSON.stringify(payload.owners)]);for(const owner of payload.owners){if(payload.devices?.[owner])await redis(['SET',DEVICE_PREFIX+owner,payload.devices[owner]]);if(payload.rules?.[owner])await redis(['SET',RULE_PREFIX+owner,payload.rules[owner]]);if(payload.states?.[owner])await redis(['SET',STATE_PREFIX+owner,payload.states[owner]]);}if(payload.legacy_store)await redis(['SET',LEGACY_STORE_KEY,payload.legacy_store]);if(payload.legacy_state)await redis(['SET',LEGACY_STATE_KEY,payload.legacy_state]);if(payload.telegram_config)await redis(['SET',TELEGRAM_CONFIG_KEY,payload.telegram_config]);console.log(`Restored encrypted alert backup for ${payload.owners.length} device(s).`);return true;}catch(error){console.log(`Alert backup restore skipped: ${error.message}`);return false;}
}
