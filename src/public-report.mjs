...
export function writePublicReport(report, output) {
  ...
  const queryStatus=report.complete?'수집 완료':report.attempt_complete?'전체 요청 완료 · 일부 조회 불가':failed.length?'재시도 필요':'부분 수집';
...
<details><summary>조회 범위와 오류 (${report.coverage.length}개 정상 조회 · ${unknown.length}개 조회 불가 · ${failed.length}개 실패)</summary><pre>${esc(JSON.stringify({coverage:report.coverage,unqueryable:unknown,failed,attempt_complete:report.attempt_complete,errors:report.errors,failure:report.failure},null,2))}</pre></details><p><a href="https://www.koreanair.com/booking/book-and-manage/award-seat-availability">대한항공 공개 조회 원문</a> · <a href="available.json">JSON 결과</a></p></main>
