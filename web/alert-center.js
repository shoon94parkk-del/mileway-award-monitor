...
async function fetchJson(url,fallback){try{const r=await timedFetch(url+(url.includes('?')?'&':'?')+'t='+Date.now(),{cache:'no-store'});return r.ok?await r.json():fallback;}catch{return fallback;}}
async function api(path,options={}){
 ...
 let r;try{r=await timedFetch(API_URL+path,{...options,headers,cache:'no-store'});}catch(error){if(error?.name==='AbortError')throw new Error('알림 서버 응답이 늦습니다. 잠시 후 다시 눌러 주세요.');throw error;}
...
async function capturePairingToken(){
 ...
 try{
  const r=await timedFetch(API_URL+'/pair',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:pairCode}),cache:'no-store'});let data={};try{data=await r.json();}catch{}
 ...
 }catch(error){toast(error?.name==='AbortError'?'알림 서버 응답이 늦습니다. 잠시 후 다시 시도해 주세요.':error.message);return false;}
}
...
function connectionMarkup(){
 ...
 if(!health)return `<section class="alert-center-connect"><div><i></i><p><b>Telegram 상태 확인 중</b><span>화면은 먼저 열렸습니다. 연결 상태를 불러오는 중입니다.</span></p></div></section>`;
...
function showAlertShell(){
 const d=$('#dialog'),content=$('#dialog-content');if(!d||!content)return false;
 content.className='alert-center-modern alert-center-direct';
 content.innerHTML='<span class="eyebrow">SEAT ALERT</span><h2 class="detail-heading">좌석 알림</h2><section class="alert-center-connect"><div><i></i><p><b>알림 화면 여는 중</b><span>연결 상태를 확인하고 있습니다.</span></p></div></section>';
 try{if(!d.open)d.showModal();return true;}catch{return false;}
}
async function openAlertCenter(){
 if(opening)return;opening=true;
 showAlertShell();
 try{await loadState();render();}catch(error){render();toast(error.message||'알림 상태를 불러오지 못했습니다.');}finally{opening=false;}
}
