...
async function testTelegram(){
 const button=$('#direct-alert-test');if(!button)return;button.disabled=true;button.textContent='전송 중…';
 try{
  const result=await api('/telegram-test',{method:'POST',body:'{}'});
  if(result?.sent){button.textContent='✓ 전송 완료';toast('✅ Telegram으로 테스트 메시지를 즉시 전송했어요.');}
  else{button.textContent='✓ 요청 완료';toast('직접 전송 준비 중이라 이번 요청은 기존 방식으로 전송됩니다.');}
  setTimeout(()=>{if(button.isConnected){button.disabled=false;button.textContent='🔔 실제 알림 테스트';}},2500);
